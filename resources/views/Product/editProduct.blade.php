                ...
                <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
                    ...
                    <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-2">
                        <div class="lg:col-span-2">
                            <form method="POST" action="{{route('product.update', ['categoryId' => $categoryId, 'productId' => $product->id])}}">
                                <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                                    @csrf
                                    <div class="wrapper md:col-span-5">
                                        <div id="survey_options">
                                            <label for="product">Nom du produit</label>
                                            <input type="text" name="product" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" value="{{old('name', $product->name)}}" />
                                        </div>
                                    </div>
